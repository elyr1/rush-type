import { ARRIVE_S, BLOOM_BIAS, BLOOM_GAIN, AUTO_HOLD_AT, BLOOM_SPREAD, CRT_BEAM, CRT_GAIN, CRT_HUM, CRT_HUM_SPEED, CRT_MASK, CRT_PITCH, CRT_SCAN, CRT_SCAN_PITCH, CYCLE_S, DRIFT_DIR, EXPOSURE, FALL, FINAL_HOLD_S, FOCAL, FONT_VAR, GLOW_BASE, GLOW_MAX_Y, GLOW_MIN, GLOW_SPEED, GLOW_SPREAD, HANG_S, HOLD_EASE_IN, HOLD_EASE_OUT, HOLD_STRENGTH, LAG_U, LEAVE_S, ORBIT_DEPTH, ORBIT_HANG_ARC, ORBIT_IN, ORBIT_OUT, ORBIT_PITCH, ORBIT_PITCH_LAG, ORBIT_RISE, ORBIT_YAW, ORBIT_YAW_LAG, PIVOT_FRAC, REST_CAP_FRAC, REST_S, RISE, ROLL_U, SAMPLES, SCROLL_DECAY, SCROLL_GAIN, SCROLL_MAX, SHUTTER_K, SHUTTER_S, SHUTTER_SHAPE, SMEAR_GAIN, SPEED_GAIN, SWAP_FLASH, SWAP_SPREAD, SX_PEAK, SY_PEAK, THIN, WORDS, orbitTurn, sampleTable, } from "./params.js";
const VERT = `
attribute vec2 aPos;
varying vec2 vUv;
void main() {
...
`;
const FRAG = (samples, bloomSpread, bloomBias) => `
...
function frameAt(tSec, sequenceLength) {
...
export class RushType {
    constructor(host) {
        ...
        this.wordHalfW = [1, 1];
        this.words = [...WORDS];
        this.sequence = makeSequence(this.words);
        this.rasteredFor = Number.NaN;
        this.family = "sans-serif";
        this.raf = 0;
        this.running = false;
        this.last = 0;
        this.startOffset = ARRIVE_END + REST_S * 0.5;
    ...
    allocAtlas() {
        ...
        if (changed) {
            ...
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, aw, hh * 2, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    ...
    rasterWord(word, half) {
        ...
        gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, half * this.hh, gl.RGBA, gl.UNSIGNED_BYTE, c);
    ...
    primeAtlas(tSec) {
        ...
        if (tau < ARRIVE_END) {
            ...
            this.rasterWord(this.sequence[previousIdx], (previousIdx % 2));
            this.rasterWord(this.sequence[k], (k % 2));
            this.rasteredFor = k - 1;
            return;
        }
        const nextIdx = (k + 1) % this.sequence.length;
        this.rasterWord(this.sequence[k], (k % 2));
        this.rasterWord(this.sequence[nextIdx], (nextIdx % 2));
        this.rasteredFor = k;
    }
    resize() {
        const gl = this.gl;
        ...
        if (w === this.canvas.width && h === this.canvas.height && this.tex)
            return;
        this.canvas.width = w;
    ...
    draw(tSec) {
        ...
        const syOf = (f) => unit * (1 + f.p * (SY_PEAK - 1));
        ...
        gl.uniform1f(this.uni.uGain, (1 + SPEED_GAIN * here.p) * (1 + SWAP_FLASH * swapSep));
        gl.uniform2f(this.uni.uSwapScl, 1 + SWAP_SPREAD * swapSep, 1 - SWAP_SPREAD * swapSep);
        gl.uniform1f(this.uni.uExp, EXPOSURE);
        gl.uniform1f(this.uni.uFocal, f);
        gl.uniform3f(this.uni.uPos, x, y, z);
        gl.uniform4f(this.uni.uRot, Math.sin(yaw), Math.cos(yaw), Math.sin(pitch), Math.cos(pitch));
        ...
        gl.uniform4f(this.uni.uGlow, gx, gy, Math.max(wordW * GLOW_SPREAD, this.canvas.width * GLOW_MIN), Math.min(Math.max(wordH * GLOW_SPREAD, this.canvas.height * GLOW_MIN), this.canvas.height * GLOW_MAX_Y));
